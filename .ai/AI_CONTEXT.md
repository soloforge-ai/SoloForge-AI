...
Cleanup Scope Reset #1 is merged and complete.

The Text Model Qualification Harness and Live Text Qualification Runner are also merged. The immediate gate is to obtain a usable live provider qualification result, keep production provider selection behind explicit owner approval, and then close only the smallest gaps required to prove one real Product-to-Post workflow.
...
- Gemini live qualification support
...
- Claude and other future providers when explicitly evaluated
...
The following remain merged, documented, or preserved but are not active roadmap drivers:
...
Generated project intelligence must not overwrite or outrank human-approved sprint state.

`docs/CURRENT_SPRINT.md` is the human-maintained source of truth for active development intent.

`docs/ROADMAP.md` is currently generated by Project Scanner and must not be treated as authoritative product intent until its ownership is corrected in a separate approved change.
...
Do not switch the production ContentEngine provider based only on a workflow success; provider qualification and owner approval are separate gates.
...
1. Complete live text-model qualification with usable evidence.
2. Keep production provider selection behind explicit owner approval.
3. Inspect the existing Product Forge path for the smallest remaining E2E gaps.
4. Run one real product through Product-to-Post.
5. Validate that the output is useful, reviewable, and manually exportable/postable.
...
- use `docs/CURRENT_SPRINT.md` as the authority for current human-approved development intent
- treat scanner output as observation, not product direction
