...
class PollinationsOAuthConfig:
    """Runtime OAuth configuration loaded from environment variables."""
    ...
    def from_env(cls) -> "PollinationsOAuthConfig":
        ...
        return cls(
            ...
            authorize_url=os.getenv(
                "POLLINATIONS_AUTHORIZE_URL", DEFAULT_AUTHORIZE_URL
            ).strip(),
            token_url=os.getenv(
                "POLLINATIONS_TOKEN_URL", DEFAULT_TOKEN_URL
            ).strip(),
...
def create_pkce_transaction() -> tuple[str, str, str]:
    """Return ``(verifier, challenge, state)`` for one OAuth transaction."""
...
def build_authorization_url(
    ...
    *,
    ...
    """Build a Pollinations authorization-code + PKCE URL."""
...
def exchange_authorization_code(
    ...
    *,
    ...
    """Exchange a single-use authorization code for a user-scoped key.

    The returned dictionary is intentionally passed through without persisting
    it. Callers should keep ``access_token`` in a secure, short-lived session
    context and never log it.
    """
