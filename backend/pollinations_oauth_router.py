...
The browser only receives opaque cookies. The Pollinations ``sk_`` token stays
server-side in an in-memory session store for this MVP.
"""
...
def pollinations_login() -> RedirectResponse:
    """Start OAuth authorization with a fresh PKCE verifier and CSRF state."""
...
def pollinations_callback(
    ...
    """Validate browser-bound state, exchange the one-time code, and create a session."""
...
def pollinations_status(session_id: str | None = Cookie(default=None, alias=_SESSION_COOKIE)) -> dict[str, object]:
    """Return connection metadata without ever returning the user's ``sk_`` token."""
...
def pollinations_logout(session_id: str | None = Cookie(default=None, alias=_SESSION_COOKIE)) -> JSONResponse:
    """Invalidate the server-side session and clear the browser cookie."""
...
def get_pollinations_access_token(session_id: str | None) -> str | None:
    """Internal helper for later generation routes; never expose this via HTTP."""
