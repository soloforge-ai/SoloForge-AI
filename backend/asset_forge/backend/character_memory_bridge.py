...
class AssetForgeCharacterMemoryBridge:
    ...
    def resolve(self, character: str) -> Optional[CharacterDNA]:
        ...
        if key in MEMORY_REQUIRED_CHARACTERS and dna is None:
            raise ValueError(
                f"Approved Character DNA is required for {character!r} but was not found."
            )
    ...
    def prompt_context(self, character: str) -> str:
        ...
        glasses_rule = (
            "- Glasses are mandatory and must never be removed."
            if dna.glasses_required
            else "- Glasses are optional unless required by the reference image."
        )
        wings_rule = (
            "- Wings may be used only when supported by the approved reference and prompt."
            if dna.wings_allowed
            else "- NO wings, halo, feathers attached to the body, or fantasy appendages."
        )
