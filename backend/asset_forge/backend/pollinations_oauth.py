...
def build_authorization_url(config: PollinationsOAuthConfig, *, code_challenge: str, state: str, models: str | None = None, budget: int | float | None = None, expiry_days: int | None = None) -> str:
    if not state:
...
def exchange_authorization_code(config: PollinationsOAuthConfig, *, code: str, code_verifier: str, timeout: int = DEFAULT_TIMEOUT_SECONDS) -> dict[str, Any]:
    if not code:
